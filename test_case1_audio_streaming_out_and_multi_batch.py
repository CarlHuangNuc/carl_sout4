...
#input_audio_path = "./translate_to_chinese.wav"
test = extract_audio_by_timestamp(input_audio_path,"./test_out.wav",0,13.0)
...
#text_ids = minicpmo_model.generate(**inputs, use_audio_in_video=USE_AUDIO_IN_VIDEO, return_audio=True)
#text_ids,audios = minicpmo_model.generate(**inputs, use_audio_in_video=USE_AUDIO_IN_VIDEO, return_audio=True)
#audios = minicpmo_model.generate(**inputs, use_audio_in_video=USE_AUDIO_IN_VIDEO, return_audio=True)
...
for wav in audios:
    ...
    full_wav.append(wav)
print(len(full_wav))
merged_wav = torch.cat(full_wav, dim=0)
sf.write("./merge.wav",merged_wav.reshape(-1).detach().cpu().numpy(),samplerate=24000,)
